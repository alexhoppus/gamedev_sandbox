...
bool PathFinder::isValid(int row, int col)
{
    ...
    // is in range
    return tm->bounds.contains(col, row);
}

// A Utility Function to check whether the given cell is
// blocked or not
bool PathFinder::isUnBlocked(int row, int col)
{
     // Returns true if the cell is not blocked else false
     Tile *t = tm->GetTile(TileCoord(col, row));
     return !t->occupied;
}
...
// to destination
void PathFinder::tracePath(Pair dest, stack<Pair> &Path)
{
    printf ("\nThe Path is ");
    int row = dest.first;
    int col = dest.second;
...
bool PathFinder::aStarSearch(Pair src, Pair dest, stack<Pair> &Path)
{
...
{
    stack<Pair> Path;
    bool res = aStarSearch(Pair(sy, sx), Pair(dsty, dstx), Path);
    if (!res)
        return 0;
    ...
    while (!Path.empty())
    {
        ...
        Path.pop();
        lpath.push_back(tm->GetTile(TileCoord(p.second, p.first)));
        printf("-> (%d,%d) ",p.first,p.second);
    }
    lpath.erase(lpath.begin());
